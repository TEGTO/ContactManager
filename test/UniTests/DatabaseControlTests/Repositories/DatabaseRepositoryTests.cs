...
namespace DatabaseControl.Tests
{
    ...
    internal class DatabaseRepositoryTests
    {
        ...
        [TestCase(3, "Test3")]
        [TestCase(4, "Test4")]
        public async Task AddRangeAsync_ValidObject_ObjectAdded(int id, string name)
        {
            // Arrange
            var dbContext = mockDbContext.Object;
            var testEntities = new[] { new TestEntity { Id = id, Name = name } };

            // Act
            await repository.AddRangeAsync(dbContext, testEntities, cancellationToken);

            // Assert
            mockDbContext.Verify(x => x.AddRangeAsync(testEntities, cancellationToken), Times.Once);
        }
